...
model = torch.load('model_FT_final_2.pth', map_location=torch.device('cpu'))
...
while True:
    ...
    if (j == 50):
        ...
        with nidaqmx.Task() as task:
            task.ai_channels.add_ai_voltage_chan("Dev2/ai1:6")
            Cal_St = task.read()
    if (j%200 == 0):
        x = time.time()
        ...
        with nidaqmx.Task() as task:
            task.ai_channels.add_ai_voltage_chan("Dev2/ai1:6")
            Real_St = task.read()
            s1 = (Real_St[0]-Cal_St[0])/6
            s2 = (Real_St[1]-Cal_St[1])/12
            s3 = (Real_St[2]-Cal_St[2])/12
            s4 = (Real_St[3]-Cal_St[3])/12
            s5 = (Real_St[4]-Cal_St[4])/12
            s6 = (Real_St[5]-Cal_St[5])/12
        ...
        ax.plot(x_data, y3_pred, 'r')
        ax2.plot(x_data, y3_data, 'b')
        ax3.plot(x_data, y1_pred, 'r')
        ax4.plot(x_data, y1_data, 'b')
        ax5.plot(x_data, y2_pred, 'r')
        ax6.plot(x_data, y2_data, 'b')
...
df.to_csv('exp_data_0704.csv', index=False)
